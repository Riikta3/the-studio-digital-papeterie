  ...
  return (
    <section className="w-full py-16 bg-[#fdf6f0]">
      <div className="max-w-md mx-auto px-4">
        <p className="text-[10px] uppercase tracking-[0.4em] text-[#c97a90]/60 font-sans mb-2 text-center">Votre réponse</p>
        <h2 className="text-3xl text-center text-[#5a3040] mb-10" style={{ fontFamily:"'Playfair Display', Georgia, serif", fontStyle:"italic" }}>
              ...
              <div className="flex gap-6">
                {[{val:"yes",label:"Avec joie ✓"},{val:"no",label:"Dans mon cœur"}].map(({val,label})=>(
                  <label key={val} className="flex items-center gap-2 text-sm text-[#5a3040]/70 font-sans cursor-pointer">
                    <input type="radio" name="attendance" value={val} checked={form.attendance===val} onChange={()=>setForm({...form,attendance:val as "yes"|"no"})} className="accent-[#c97a90]" />
                    {label}
                  </label>
                ))}
              </div>
