...
const FlipUnit = ({ value, label }: { value: number; label: string }) => {
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      <span className="text-[9px] sm:text-[11px] uppercase tracking-[0.25em] text-foreground/80 font-sans font-medium">{label}</span>
    </div>
  );
};
  ...
  return (
    <section className="w-full pb-20 bg-background overflow-hidden">
      <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.9, ease: "easeOut" }}
        ...
        <p className="text-[11px] sm:text-xs uppercase tracking-[0.4em] text-primary/60 font-sans">Le grand jour approche</p>
        ...
        <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-6 md:gap-10">
          {[{v:time.days,l:"Jours"},{v:time.hours,l:"Heures"},{v:time.minutes,l:"Minutes"},{v:time.seconds,l:"Secondes"}].map((block) => (
            <FlipUnit key={block.l} value={block.v} label={block.l} />
          ))}
        </div>
        ...
        <div className="flex flex-col items-center gap-8 mt-4 w-full max-w-sm">
          <div className="flex items-center gap-4 w-full">
            ...
            <span className="text-[11px] sm:text-xs uppercase tracking-[0.2em] text-foreground/50 font-sans whitespace-nowrap">{formattedDate}</span>
            <div className="flex-1 h-px bg-primary/20" />
          </div>
          ...
          <div ref={calRef} className="relative z-30">
            <button onClick={() => setCalOpen(v => !v)}
              ...
              <ChevronDown className={`w-3.5 h-3.5 text-primary/50 transition-transform ${calOpen?"rotate-180":""}`} />
            </button>
            {calOpen && (
              <motion.div initial={{ opacity: 0, y: -10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -10, scale: 0.95 }}
                ...
                <button onClick={() => { window.open(googleUrl,"_blank"); setCalOpen(false); }}
                  className="w-full px-5 py-3.5 text-left text-sm text-foreground hover:bg-primary/5 transition-colors border-b border-primary/10 font-medium">Google Calendar</button>
                <button onClick={() => {
                ...
                }} className="w-full px-5 py-3.5 text-left text-sm text-foreground hover:bg-primary/5 transition-colors font-medium">Apple / Outlook (.ics)</button>
              </motion.div>
            )}
          </div>
        </div>
