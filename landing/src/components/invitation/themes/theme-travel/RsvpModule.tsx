...
import { Heart, Plane, HeartCrack } from "lucide-react";
...
export function RsvpModule({ weddingId, isDemo = false }: ModuleProps) {
  const [form, setForm] = useState({
    ...
    guests: "0", dietary: [] as string[], message: "",
  });
            ...
            {status === "success" ? (
              ...
              >
                ...
                {form.attendance === "yes" && (
                  ...
                  >
                    <select className={selectClass} value={form.guests} onChange={e => setForm({ ...form, guests: e.target.value })}>
                      {[0, 1, 2, 3, 4].map(n => (
                        <option key={n} value={n}>{n === 0 ? "Venu(e) seul(e)" : `+${n} accompagnant${n > 1 ? "s" : ""}`}</option>
                      ))}
                    </select>
