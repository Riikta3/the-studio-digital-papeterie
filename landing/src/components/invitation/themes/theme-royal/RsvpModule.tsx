...
export function RsvpModule({ weddingId, isDemo = false }: ModuleProps) {
  ...
  const inputClass = "w-full bg-transparent border-b border-[#c4a23a]/20 py-3 text-sm text-[#1e3a8a] placeholder:text-[#1e3a8a]/30 focus:outline-none focus:border-[#c4a23a]/50 transition-colors font-sans";

  return (
    <section className="w-full py-16 bg-[#eef2ff]">
      <div className="max-w-md mx-auto px-4">
        <p className="text-[10px] uppercase tracking-[0.4em] text-[#c4a23a]/60 font-sans mb-2 text-center">Votre réponse</p>
        <h2 className="text-3xl text-center text-[#1e3a8a] mb-10" style={{ fontFamily:"'Playfair Display', Georgia, serif", fontStyle:"italic" }}>
        ...
        <AnimatePresence mode="wait">
          {status==="success" ? (
            <motion.div key="success" initial={{ opacity:0 }} animate={{ opacity:1 }} className="text-center flex flex-col items-center gap-4">
              <Heart className="w-8 h-8 text-[#c4a23a]" fill="currentColor" />
              <p className="text-[#1e3a8a] font-sans text-sm">Merci pour votre réponse.</p>
            </motion.div>
          ) : (
            <motion.form key="form" initial={{ opacity:0 }} animate={{ opacity:1 }} onSubmit={handleSubmit} className="flex flex-col gap-6">
              ...
              <div className="flex gap-6">
                {[{val:"yes",label:"Avec joie ✓"},{val:"no",label:"Dans mon cœur"}].map(({val,label})=>(
                  <label key={val} className="flex items-center gap-2 text-sm text-[#1e3a8a]/70 font-sans cursor-pointer">
                    <input type="radio" name="attendance" value={val} checked={form.attendance===val} onChange={()=>setForm({...form,attendance:val as "yes"|"no"})} className="accent-[#c4a23a]" />
                    {label}
                  </label>
                ))}
              </div>
              ...
              <button type="submit" disabled={status==="submitting"||!form.attendance}
                className="self-center px-8 py-3 rounded-full border border-[#c4a23a]/40 text-[#1e3a8a] text-[11px] uppercase tracking-[0.3em] font-sans hover:bg-[#c4a23a]/10 disabled:opacity-30 transition-colors mt-2">
