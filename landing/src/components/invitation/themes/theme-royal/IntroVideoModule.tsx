  ...
  return (
    <section className='w-full'>
      ...
      >
        ...
        <h3 className='font-heading text-5xl md:text-6xl italic text-[#1e3a8a] mb-4'>
          {data.subtitle}
        </h3>
        <p className='text-[#1e3a8a]/60 text-sm md:text-base max-w-2xl mx-auto leading-relaxed font-light mb-12'>
          {data.description}
        </p>

        <div className='relative w-full max-w-3xl mx-auto aspect-video rounded-[2rem] overflow-hidden border border-[#c4a23a]/20 shadow-xl bg-secondary group'>
          <AnimatePresence mode='wait'>
            {!isPlaying ? (
              ...
              >
                ...
                <div className='relative z-10 w-20 h-20 md:w-24 md:h-24 bg-card/80 backdrop-blur-sm rounded-full flex items-center justify-center shadow-sm border border-[#c4a23a]/20 group-hover:scale-110 transition-transform duration-500 ease-out'>
                  <Play
                    className='w-8 h-8 md:w-10 md:h-10 text-[#c4a23a] ml-1 opacity-80'
                    strokeWidth={1}
                  />
                </div>

                <span className='relative z-10 mt-6 text-[10px] md:text-xs font-bold uppercase tracking-[0.2em] text-[#1e3a8a]/60 opacity-70 group-hover:opacity-100 transition-opacity duration-300'>
                  Lancer la vidéo
                </span>
              </motion.div>
            ) : (
