...
export function CountdownModule({ weddingDate, partner1, partner2 }: CountdownModuleProps) {
  ...
  return (
    <section className="w-full py-16 bg-[#fdf6f0]">
      <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.9, ease: "easeOut" }}
        ...
        <p className="text-[10px] uppercase tracking-[0.4em] text-[#c97a90]/60 font-sans">Le grand jour approche</p>
        <div className="flex items-start gap-8 sm:gap-12">
          {[{ v: time.days, l: "Jours" }, { v: time.hours, l: "Heures" }, { v: time.minutes, l: "Minutes" }, { v: time.seconds, l: "Secondes" }].map((block) => (
            <div key={block.l} className="flex flex-col items-center gap-2">
              ...
              <span className="text-[9px] uppercase tracking-[0.25em] text-[#c97a90]/50 font-sans">{block.l}</span>
            </div>
          ))}
        </div>
        <div className="flex items-center gap-4 w-full max-w-xs">
          <div className="flex-1 h-px bg-[#c97a90]/20" />
          <span className="text-[10px] uppercase tracking-[0.2em] text-[#5a3040]/50 font-sans whitespace-nowrap">{formattedDate}</span>
          <div className="flex-1 h-px bg-[#c97a90]/20" />
        </div>
          ...
          {calOpen && (
            <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }}
              ...
              <button onClick={() => { window.open(googleUrl, "_blank"); setCalOpen(false); }}
                className="w-full px-4 py-3 text-left text-sm text-[#5a3040] hover:bg-[#fdf6f0] transition-colors border-b border-[#c97a90]/10">Google Calendar</button>
              <button onClick={() => {
                ...
                setCalOpen(false);
              }} className="w-full px-4 py-3 text-left text-sm text-[#5a3040] hover:bg-[#fdf6f0] transition-colors">Apple / Outlook (.ics)</button>
            </motion.div>
          )}
        </div>
      </motion.div>
