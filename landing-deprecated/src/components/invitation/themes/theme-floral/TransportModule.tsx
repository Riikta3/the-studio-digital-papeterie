...
const getIcon = (type: TransportIconType) => {
  switch (type) {
    case "Train":
      return <Train className='w-6 h-6 text-[#c97a90]' strokeWidth={1.5} />;
    case "Plane":
      return <Plane className='w-6 h-6 text-[#c97a90]' strokeWidth={1.5} />;
    case "Bus":
      return <Bus className='w-6 h-6 text-[#c97a90]' strokeWidth={1.5} />;
    case "Car":
      return <Car className='w-6 h-6 text-[#c97a90]' strokeWidth={1.5} />;
    case "Ship":
      return <Ship className='w-6 h-6 text-[#c97a90]' strokeWidth={1.5} />;
    default:
      return <Car className='w-6 h-6 text-[#c97a90]' strokeWidth={1.5} />;
  }
};
  ...
  return (
    <section className='w-full'>
      <div className='text-center mb-20 space-y-4'>
        <h2 className='text-xs md:text-sm font-bold uppercase tracking-[0.2em] text-[#5a3040]/60'>
          Logistique
        </h2>
        <h3 className='font-heading text-5xl md:text-6xl italic text-[#5a3040]'>
          Votre Trajet
        </h3>
      </div>
      ...
      <div className='grid md:grid-cols-12 gap-8 max-w-5xl mx-auto'>
        ...
        {data.options.length > 0 && (
          ...
          >
            {data.options.map((option) => (
              <div
                key={option.id}
                className='group flex flex-col sm:flex-row gap-8 items-start sm:items-center bg-white p-10 py-12 rounded-[2.5rem] border border-[#c97a90]/20 shadow-xl hover:border-primary/30 transition-colors duration-300'
              >
                ...
                <div className='flex flex-col justify-center'>
                  <h4 className='font-heading text-3xl text-[#5a3040] mb-3'>
                    {option.title}
                  </h4>
                  <p className='text-[#5a3040]/60 font-light text-[15px] leading-relaxed max-w-md'>
                    {option.description}
                  </p>
                </div>
              </div>
        ...
        {data.carpoolUrl && (
          <motion.div
            ...
            className={`relative overflow-hidden bg-secondary rounded-[3rem] border border-[#c97a90]/20 shadow-[0_8px_30px_-12px_rgba(0,0,0,0.04)] flex flex-col items-center text-center p-12 lg:p-14 ${data.options.length > 0 ? "md:col-span-5" : "col-span-12 max-w-md mx-auto w-full"}`}
          >
            <div
            ...
            <div className='relative z-10 w-full flex flex-col items-center h-full'>
              <div className='w-20 h-20 bg-white rounded-full flex items-center justify-center shadow-sm border border-[#c97a90]/20 mt-4 mb-10'>
                <Car className='w-7 h-7 text-[#c97a90]' strokeWidth={1.5} />
              </div>

              <h4 className='font-heading text-4xl text-[#5a3040] mb-6'>
                Covoiturage
              </h4>

              <p className='text-[#5a3040]/60 font-light text-[15px] leading-relaxed mb-auto pb-12 max-w-[280px]'>
              ...
              <div className='w-full flex justify-center pb-2'>
                <Button
                  asChild
                  size='lg'
                  className='rounded-full h-auto py-3.5 px-8 whitespace-normal text-center text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.15em] border border-[#c97a90]/20 bg-white text-[#5a3040]/60 hover:bg-muted hover:text-primary hover:border-primary/30 transition-all gap-2 shadow-sm'
                >
                  <a
