    ...
    <section className='w-full'>
      ...
      >
        ...
        <div className='text-center mb-12'>
          <h2 className='font-heading text-4xl md:text-5xl text-[#5a3040]'>
            Questions{" "}
            <span className='italic text-[#c97a90] opacity-80'>Fréquentes</span>
          </h2>
        </div>
        ...
        <div className='space-y-4'>
          {data.questions.map((faq, index) => (
            <motion.div
              ...
              className='bg-white rounded-[1.5rem] border border-[#c97a90]/20 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)] overflow-hidden transition-all duration-300'
            >
              <button
                onClick={() => setOpenIndex(openIndex === index ? null : index)}
                className='flex w-full items-center justify-between p-7 text-left transition-colors hover:bg-muted'
              >
                <span className='font-heading text-xl md:text-2xl text-[#5a3040] pr-8 leading-snug'>
                  {faq.question}
                </span>
                <div
                  className={cn(
                    ...
                    openIndex === index
                      ? "bg-[#c97a90] border-[#c97a90] text-white rotate-180"
                      : "border-primary/30 text-[#c97a90]",
                  )}
                >
                  {openIndex === index ? (
                ...
                {openIndex === index && (
                  ...
                  >
                    <div className='px-7 pb-8 pt-0'>
                      ...
                      <p className='text-[#5a3040]/60 text-base leading-relaxed font-light'>
                        {faq.answer}
                      </p>
                    </div>
                  </motion.div>
