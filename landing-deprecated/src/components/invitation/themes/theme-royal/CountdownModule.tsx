...
export function CountdownModule({ weddingDate, partner1, partner2 }: ModuleProps) {
  ...
  return (
    <section className="w-full py-16 bg-[#eef2ff]">
      <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.9, ease: "easeOut" }}
        ...
        <p className="text-[10px] uppercase tracking-[0.4em] text-[#c4a23a]/60 font-sans">Le grand jour approche</p>
          ...
          {[{v:time.days,l:"Jours"},{v:time.hours,l:"Heures"},{v:time.minutes,l:"Minutes"},{v:time.seconds,l:"Secondes"}].map((block) => (
            <div key={block.l} className="flex flex-col items-center gap-2">
              <motion.span key={block.v} initial={{ opacity: 0.6, y: 4 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}
                className="text-5xl sm:text-6xl text-[#1e3a8a] tabular-nums"
                style={{ fontFamily: "'Playfair Display', Georgia, serif", fontStyle: "italic" }}>
                {pad(block.v)}
              </motion.span>
              <span className="text-[9px] uppercase tracking-[0.25em] text-[#c4a23a]/50 font-sans">{block.l}</span>
            </div>
          ))}
        </div>
        <div className="flex items-center gap-4 w-full max-w-xs">
          <div className="flex-1 h-px bg-[#c4a23a]/20" />
          <span className="text-[10px] uppercase tracking-[0.2em] text-[#1e3a8a]/50 font-sans whitespace-nowrap">{formattedDate}</span>
          <div className="flex-1 h-px bg-[#c4a23a]/20" />
        </div>
        <div ref={calRef} className="relative">
          <button onClick={() => setCalOpen(v => !v)}
            className="flex items-center gap-2 px-6 py-3 rounded-full border border-[#c4a23a]/30 text-[#1e3a8a] text-[11px] uppercase tracking-[0.2em] font-sans hover:bg-[#c4a23a]/5 transition-colors">
            <CalendarPlus className="w-3.5 h-3.5 text-[#c4a23a]" />
            Ajouter à mon agenda
            <ChevronDown className={`w-3 h-3 text-[#c4a23a]/50 transition-transform ${calOpen?"rotate-180":""}`} />
          </button>
          {calOpen && (
            <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }}
              className="absolute top-full mt-2 left-1/2 -translate-x-1/2 w-48 bg-white border border-[#c4a23a]/20 rounded-2xl shadow-xl overflow-hidden z-50">
              <button onClick={() => { window.open(googleUrl,"_blank"); setCalOpen(false); }}
                className="w-full px-4 py-3 text-left text-sm text-[#1e3a8a] hover:bg-[#eef2ff] transition-colors border-b border-[#c4a23a]/10">Google Calendar</button>
              <button onClick={() => {
              ...
              }} className="w-full px-4 py-3 text-left text-sm text-[#1e3a8a] hover:bg-[#eef2ff] transition-colors">Apple / Outlook (.ics)</button>
            </motion.div>
          )}
        </div>
      </motion.div>
