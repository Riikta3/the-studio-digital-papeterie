    ...
    <section className='w-full'>
      ...
      >
        <div className='text-center mb-16 space-y-4'>
          ...
          <p className='text-[#4a3728]/60 text-sm md:text-base max-w-2xl mx-auto leading-relaxed font-light'>
            {data.description}
          </p>
        </div>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8'>
          {data.options.map((option, index) => (
            <motion.div
              ...
              className='bg-white rounded-[2rem] p-8 border border-[#a98467]/20 shadow-xl flex flex-col h-full group hover:border-primary/30 transition-colors'
            >
              <div className='w-14 h-14 bg-secondary rounded-full flex items-center justify-center mb-6 text-[#a98467]'>
                {getIcon(option.type)}
              </div>

              <h4 className='font-heading text-3xl text-[#4a3728] mb-3 leading-tight'>
                {option.name}
              </h4>

              <div className='flex items-center gap-2 text-[10px] md:text-xs font-bold text-[#4a3728]/60 uppercase tracking-widest mb-4'>
                <MapPin className='w-3.5 h-3.5 opacity-70' /> {option.distance}
              </div>

              <p className='text-[#4a3728]/60 text-[14px] leading-relaxed font-light mb-8 flex-grow'>
                {option.description}
              </p>
              ...
              {option.url && (
                <div className='pt-2 mt-auto'>
                  <Button
                    asChild
                    variant='outline'
                    className='w-full rounded-full h-auto py-3.5 px-4 whitespace-normal text-center text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.15em] border-[#a98467]/20 bg-white text-[#4a3728]/60 hover:bg-muted hover:text-primary hover:border-primary/30 transition-all gap-2'
                  >
                    <a
                      href={option.url}
                      target='_blank'
